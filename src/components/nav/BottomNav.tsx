...
export function BottomNav({
  ...
  return (
    ...
    >
      <ul className="grid h-[4.25rem] grid-cols-5 px-1 pt-1">
        {items.map((item) => {
            ...
            return (
              <li key={item.key} className="flex items-start justify-center">
                <Link
                  ...
                  className="nav-fab relative z-10 -mt-6 flex h-14 w-14 items-center justify-center rounded-full bg-accent text-ink ring-4 ring-canvas transition-[transform] duration-[var(--duration-press)] ease-[var(--ease-out)] active:scale-[0.97]"
                >
                  <NavIcon name="create" size={28} />
          ...
          return (
            <li key={item.key} className="flex items-center justify-center">
              <Link
                href={href}
                aria-current={on ? "page" : undefined}
                className={`relative flex min-h-12 min-w-[44px] flex-col items-center justify-center gap-1 transition-[color] duration-[var(--duration-press)] ease-[var(--ease-out)] ${
