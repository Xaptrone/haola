# Marketing

Primary page `/` sells the marketplace to **business owners**.

## Job

In under 10 seconds: virtual KOLs make content for my business. One action: Start a campaign.

## Structure

1. Hero — one promise, purple CTA, brand-safe / QC line
2. Wow — vertical virtual-KOL video
3. How it works — tell AI → match KOLs → approve content
4. Trust — QC, claim checks, you approve before publish
5. Proof — sample AMF / match cards labelled Predicted
6. Close CTA

## Guest AI

CTA opens the campaign prompt. One or two clarify questions, then sample brief + recommended KOL cards. Continue → Google login → draft attaches to the new **business workspace**.

Do not lead with Book a demo. Groups/chains may have a muted contact link.

## Creators

Quiet “I’m a creator” → `/creators` → `/register/creator`. Account, then creator card, then studio. Full creator marketing later.
