...
name: haola-uiux
description: >-
  Project fxgen (Haola) UI/UX router. Use when designing, polishing, previewing,
  or building marketing, creator studio, business workspace, dashboards,
  Tailwind, components, landing pages, or mobile/desktop shells. Dark canvas,
  purple accent (#6F43F6), Sora, video-first, AI action cards. Tokens in
  DESIGN.md win over upstream skill palettes.
---
...
| Accent | `--color-accent` `#6F43F6` | 10% spark |

- Logo: fxgen wordmark (`BrandLogo`, `/fxgen-logo-dark.png`). Purple **g** is the accent source.
...
| --- | --- | --- |
| `/` landing | Persuade | Logo + headline + 9:16 reel + one purple CTA |
...
If an upstream skill wants bone paper, Inter, multi-accent purple mesh, three equal feature cards, or a second accent — **ignore it**. fxgen tokens stay (single purple spark only).

## Preview standard

Squint test: ~60% dark air, ~30% frames, one purple spark.
